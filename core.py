...
def spatial_corr(input_filename1,input_filename2,mask_filename=None,type='pearson'):
    ...
    if mask_filename:
        input3 = nib.load(mask_filename)
        input3_d = input3.get_data()
    
        mask_coords = np.nonzero(input3_d)
        if type is 'spearman':
            [input_corr, p] = scipy.stats.spearmanr(input1_d[mask_coords],input2_d[mask_coords])
        else:
            input_corr = np.corrcoef(input1_d[mask_coords],input2_d[mask_coords])
            input_corr = input_corr[0,1]
    else:
        if type is 'spearman':
            [input_corr, p] = scipy.stats.spearmanr(input1_d.flatten(),input2_d.flatten())
        else:
            input_corr = np.corrcoef(input1_d.flatten(),input2_d.flatten())
            input_corr= input_corr[0,1]
    
    print('spatial correlation: r=%1.4f'%(input_corr))
    return input_corr